...
- Automatic *Treesitter* (optional.) All you need is libtree-sitter-dev and emacs 29 compiled --with-tree-sitter support
...
- Nix (rnix-lsp)
- Ocaml (ocaml-lsp)
- Php (php-language-server) Note: ~M-x eglot~ manually or use ~(setq spartan-php-langserver t)~ for autostarting eglot
- Python (pylsp)
